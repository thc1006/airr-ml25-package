#!/usr/bin/env python3
"""
main_v2.py - AIRR-ML-25 Enhanced Feature Engineering (Priority 1)
==================================================================

Enhanced version of main.py with Priority 1 features:
...
All features are memory-efficient with sparse representations where possible.
...
Competition Target: Score > 0.82 to beat current leader (0.81364)
"""
...
# ============================================================================
# Feature Extraction Functions (Priority 1)
# ============================================================================

def extract_multiscale_kmers(df: pd.DataFrame, k_values: List[int] = [3, 4, 5]) -> Dict[str, int]:
    """
    Extract k-mer counts for multiple k values with prefixes.
    ...
    Returns:
        Dictionary with prefixed k-mer counts (e.g., 'k3_CAR', 'k4_CARS', 'k5_CARSL')
    """
    all_kmers = {}
...
def extract_vj_features(df: pd.DataFrame,
                       top_v: int = 20,
                       top_j: int = 20,
                       top_vj_pairs: int = 50) -> Dict[str, float]:
    """
    Extract V/J gene usage features.

    Args:
        df: DataFrame with 'v_call' and 'j_call' columns
        top_v: Number of top V genes to include
        top_j: Number of top J genes to include
        top_vj_pairs: Number of top VJ pairs to include

    Returns:
        Dictionary with V/J usage features
    """
    features = {}
    total_seqs = len(df)
    ...
    if 'v_call' in df.columns and 'j_call' in df.columns:
        ...
        for i, ((v, j), count) in enumerate(vj_pairs_sorted.head(top_vj_pairs).items()):
            # Sanitize gene names for feature names
            v_clean = str(v).replace('*', '_').replace('/', '_')
            j_clean = str(j).replace('*', '_').replace('/', '_')
            features[f'vj_pair_{v_clean}_{j_clean}'] = count / total_seqs
...
def extract_clonality_features(df: pd.DataFrame) -> Dict[str, float]:
    """
    Extract clonality metrics: Shannon entropy, Gini-Simpson, D50, clonality score.

    Args:
        df: DataFrame with sequence data

    Returns:
        Dictionary with clonality features
    """
    features = {}

    # Count sequence frequencies (clones)
    ...
    features['shannon_entropy'] = shannon_ent

    # Gini-Simpson index (diversity)
    gini_simpson = 1.0 - np.sum(frequencies ** 2)
    features['gini_simpson'] = gini_simpson

    # D50: Number of clones making up 50% of repertoire
    ...
    if max_entropy > 0:
        ...
        clonality = 0.0
    features['clonality_score'] = clonality

    return features


def extract_cdr3_length_features(df: pd.DataFrame) -> Dict[str, float]:
    """
    ...
    Returns:
        Dictionary with length statistics features
    """
    features = {}

    if 'junction_aa' not in df.columns or len(df) == 0:
        return {
            'cdr3_length_mean': 0.0,
            'cdr3_length_std': 0.0,
            'cdr3_length_median': 0.0,
            'cdr3_length_q25': 0.0,
            'cdr3_length_q75': 0.0,
            'cdr3_length_skewness': 0.0,
            'cdr3_length_kurtosis': 0.0
        }
    ...
    try:
        features['cdr3_length_skewness'] = skew(lengths)
        features['cdr3_length_kurtosis'] = kurtosis(lengths)
    except:
        features['cdr3_length_skewness'] = 0.0
        features['cdr3_length_kurtosis'] = 0.0

    return features


def extract_all_features(df: pd.DataFrame, k_values: List[int] = [3, 4, 5]) -> Dict[str, float]:
    """
    Extract all Priority 1 features from a repertoire.

    Args:
        df: DataFrame with repertoire data
        k_values: List of k-mer sizes for multi-scale extraction
    ...
    # Multi-scale k-mers
    features.update(extract_multiscale_kmers(df, k_values=k_values))
    ...
    # CDR3 length
    features.update(extract_cdr3_length_features(df))

    return features


def load_and_encode_features(data_dir: str,
                            k_values: List[int] = [3, 4, 5]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Memory-efficient feature encoding of repertoire data with Priority 1 features.
    ...
    for item in tqdm(data_loader, total=total_files, desc=f"Extracting Priority 1 features"):
        if os.path.exists(metadata_path):
        ...
        features = extract_all_features(data_df, k_values=k_values)
        features['ID'] = rep_id
...
# ============================================================================
# File I/O and Dataset Management
# ============================================================================
...
# ============================================================================
# Enhanced Classifier - Core ML Model
# ============================================================================

class EnhancedKmerClassifier:
    """
    Enhanced L1-regularized logistic regression for multi-scale k-mer and feature data.
    ...
    def score_all_sequences(self, sequences_df, sequence_col='junction_aa'):
        """
        Score all sequences using model coefficients for multi-scale k-mers.
        ...
        # Build k-mer to index mapping for each scale
        kmer_to_index = {}
        for fname in self.feature_names_:
            # Check if this is a k-mer feature (starts with k3_, k4_, k5_)
                ...
                if len(seq) >= k:
                    for i in range(len(seq) - k + 1):
                        ...
                        if kmer_key in kmer_to_index:
                            feature_vec[kmer_to_index[kmer_key]] = 1.0  # Binary presence
...
# ============================================================================
# ImmuneStatePredictor - Official Template Class (Enhanced)
# ============================================================================

class ImmuneStatePredictor:
    """
    Enhanced template-compliant predictor for AIRR-ML-25 with Priority 1 features.
    ...
    def __init__(self, n_jobs: int = -1, device: str = 'cpu',
        ...
        print(f"Initialized Enhanced Predictor:")
        print(f"  - n_jobs: {self.n_jobs}")
        print(f"  - device: {self.device}")
        print(f"  - k-mer scales: {self.k_values}")
        print(f"  - Priority 1 features: ENABLED")
    ...
    def fit(self, train_dir_path: str):
        ...
        """
        print(f"\n{'='*70}")
        print(f"TRAINING: {train_dir_path}")
        print(f"{'='*70}")

        # Load and encode all features
        X_train_df, y_train_df = load_and_encode_features(
            train_dir_path, k_values=self.k_values
        )

        print(f"Extracted features: {X_train_df.shape[1]} total")

        # Count feature types
        kmer_features = sum(1 for c in X_train_df.columns if c.startswith('k'))
        vj_features = sum(1 for c in X_train_df.columns if 'usage' in c or 'vj_pair' in c)
        clonality_features = sum(1 for c in X_train_df.columns if c in [
            'shannon_entropy', 'gini_simpson', 'd50', 'clonality_score'
        ])
        length_features = sum(1 for c in X_train_df.columns if 'cdr3_length' in c)

        print(f"  - Multi-scale k-mers: {kmer_features}")
        ...
        self.model = EnhancedKmerClassifier(
            c_values=[1.0, 0.2, 0.1, 0.05, 0.03],
            cv_folds=5,
    ...
    def predict_proba(self, test_dir_path: str) -> pd.DataFrame:
        ...
        X_test_df, _ = load_and_encode_features(test_dir_path, k_values=self.k_values)
    ...
    def identify_associated_sequences(self, train_dir_path: str, top_k: int = 50000) -> pd.DataFrame:
        ...
        print(f"\nIdentifying {top_k} important sequences for {dataset_name}...")
...
def process_single_dataset(train_dir: str, test_dirs: List[str], out_dir: str,
                           n_jobs: int, device: str, k_values: List[int] = [3, 4, 5]) -> None:
    """Process a single train-test dataset pair."""
    validate_dirs_and_files(train_dir, test_dirs, out_dir)

    predictor = ImmuneStatePredictor(n_jobs=n_jobs, device=device, k_values=k_values)
...
def run_all_datasets(train_root: str, test_root: str, out_dir: str,
    ...
    print(f"\n{'#'*70}")
    print("AIRR-ML-25: Enhanced Pipeline with Priority 1 Features")
    print(f"{'#'*70}")
    ...
    for train_dir, test_dirs in dataset_pairs:
        if not test_dirs:
            ...
            continue
        process_single_dataset(train_dir, test_dirs, out_dir, n_jobs, device, k_values)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="AIRR-ML-25 Enhanced Predictor (Priority 1 Features)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single dataset with default k-mers (3,4,5)
  python main_v2.py --train_dir ./data/train_datasets/train_dataset_1 \\
                    --test_dirs ./data/test_datasets/test_dataset_1 \\
                    --out_dir ./results --n_jobs 8

  # All datasets with custom k-mer sizes
  python main_v2.py --train_root ./data/train_datasets \\
    ...
    parser.add_argument('--k_values', type=int, nargs='+', default=[3, 4, 5],
                        help='K-mer sizes for multi-scale extraction (default: 3 4 5)')
...
def main():
    ...
    if args.train_root and args.test_root:
        ...
        run_all_datasets(args.train_root, args.test_root, args.out_dir,
                        args.n_jobs, args.device, args.k_values)
