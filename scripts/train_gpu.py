#!/usr/bin/env python3
"""
AIRR-ML-25 GPU-Optimized Championship Pipeline
===============================================
Memory-efficient GPU XGBoost with k=3 k-mers only.
- Uses sparse matrices to reduce memory
- Limits features to fit in 16GB VRAM
...
# =============================================================================
# Feature Extraction - k=3 only for memory efficiency
# =============================================================================

def extract_kmer_counts(sequences: List[str], k: int = 3) -> Dict[str, float]:
    ...
    # Normalize
    if total > 0:
        return {f"k{k}_{kmer}": count / total for kmer, count in counts.items()}
    return {}


def load_repertoire(file_path: str, repertoire_id: str, k: int = 3) -> Tuple[str, Dict]:
...
def load_all_repertoires(data_dir: str, k: int = 3, n_jobs: int = 8) -> Tuple:
    ...
    print(f"  Shape: {X.shape} (sparse, {X.nnz} non-zeros)")
...
# =============================================================================
# GPU XGBoost Predictor with Memory Optimization
# =============================================================================

class GPUOptimizedPredictor:
    """GPU-accelerated XGBoost predictor with memory optimization."""
    ...
    def fit(self, train_dir: str):
        ...
        # Load data
        X, y, rep_ids, self.vectorizer = load_all_repertoires(train_dir, k=self.k, n_jobs=self.n_jobs)
        ...
        # Train XGBoost with GPU - memory optimized parameters
        print(f"  Training XGBoost with GPU (memory optimized)...")

        # Convert sparse matrix to DMatrix efficiently
        dtrain = xgb.DMatrix(X_train, label=y_train)
        dval = xgb.DMatrix(X_val, label=y_val)

        # Memory-optimized GPU parameters
        params = {
    ...
    def predict_proba(self, test_dir: str) -> pd.DataFrame:
        ...
        for file_path in tqdm(test_files, desc="Predicting", disable=False):
            ...
            _, features = load_repertoire(file_path, rep_id, k=self.k)
...
def main():
    print("=" * 60)
    print("AIRR-ML-25 GPU-Optimized Championship Pipeline")
    print("=" * 60)
    ...
    out_dir = Path("./results_gpu_optimized")
    ...
    for train_dir in train_datasets:
        ...
        predictor = GPUOptimizedPredictor(k=3, n_jobs=8)
    ...
    print("\n" + "=" * 60)
    print("GPU-Optimized Championship Pipeline Complete!")
    print("=" * 60)
