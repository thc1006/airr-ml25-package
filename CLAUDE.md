...
永遠不得用 CPU 進行任何最終生成遞交的檔案的的任何機器學習、深度學習訓練。每一次的分數都要比上一次的高，絕對不可以出現要使用者繳交相等成績或是更低成績的事情發生。

目標是要奪冠，因此不要浪費在沒有意義的微調分數浪費遞交機會上。
...
> Note: Filesystem MCP not needed — Claude Code has built-in file tools (Read, Write, Edit, Glob, Grep)
...
|-----------|---------------|-------|
| **CPU** | AMD Ryzen 7 7800X3D 8-Core | 3D V-Cache for gaming/data workloads |
| **RAM** | 32 GB DDR5 | Sufficient for most datasets |
| **GPU** | NVIDIA RTX 5080 16GB VRAM | Driver 580.95.05 |
| **Storage** | ~104 GB available | SSD recommended |

### 9.2 Development Tools

| Tool | Version | Path | Purpose |
|------|---------|------|---------|
| **Python** | 3.10.12 | `/usr/bin/python3` | Runtime |
| **uv** | 0.9.15 | `~/.local/bin/uv` | Fast Python package manager (10-100x faster than pip) |
| **ripgrep (rg)** | 13.0.0 | `/usr/bin/rg` | Fast code search |
...
rg -C3 "KmerClassifier" --type py

# Search for exact string
rg -F "junction_aa" --type py
...
### 13.6 Debugging & Operations Agents
...
### 13.7 Agent Invocation Examples
...
### 13.8 Agent Collaboration Patterns
