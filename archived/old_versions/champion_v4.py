#!/usr/bin/env python3
"""
Champion V4 - GPU-Optimized Feature Pipeline
=============================================
Fixes GPU OOM by limiting k-mers to top 5000 most common.
- K-mers: Top 5000 (instead of all ~150K)
- V/J genes: All unique genes from data
- Diversity metrics: 11 features
"""
...
TOP_K = 50000
KMER_K = 4
MAX_KMERS = 5000  # Limit k-mers to top 5000 most common
...
# ============================================================================
# Feature Building - Limited K-mer Vocabulary
# ============================================================================
def collect_feature_vocabulary(train_datasets: List[int]) -> Dict[str, List[str]]:
    """Collect feature vocabulary with LIMITED k-mers (top 5000)."""
    print("\n" + "=" * 60)
    print("Phase 1: Collecting Feature Vocabulary (GPU-Optimized)")
    print("=" * 60)

    all_v_genes = set()
    all_j_genes = set()
    kmer_global_counts = Counter()
    ...
    # Select top MAX_KMERS k-mers
    top_kmers = [kmer for kmer, _ in kmer_global_counts.most_common(MAX_KMERS)]
    kmer_features = sorted([f'kmer_{k}' for k in top_kmers])
    ...
    print(f"\n  Feature Vocabulary Summary (GPU-Optimized):")
    print(f"    V genes: {len(v_features)}")
    print(f"    J genes: {len(j_features)}")
    print(f"    K-mers: {len(kmer_features)} (top {MAX_KMERS} from {len(kmer_global_counts)} total)")
    ...
    return {
        ...
        'kmer_set': set(top_kmers)  # For fast lookup during extraction
    }
...
# Feature Extraction
# ============================================================================
def extract_features(df: pd.DataFrame, feature_names: List[str], kmer_set: set = None) -> np.ndarray:
    ...
    # === K-mer features (only for k-mers in vocabulary) ===
    if 'junction_aa' in df.columns:
        seqs = df['junction_aa'].dropna().astype(str)
        kmer_counts = Counter()
        total_kmers = 0
        ...
        for seq in seqs:
            for i in range(len(seq) - KMER_K + 1):
                kmer = seq[i:i+KMER_K]
                if kmer.isalpha() and (kmer_set is None or kmer in kmer_set):
                    kmer_counts[kmer] += 1
                    total_kmers += 1
...
def extract_dataset(dataset_path: Path, feature_names: List[str], kmer_set: set,
    ...
    if is_test:
        ...
        for tsv_file in tqdm(tsv_files, desc=f"  {dataset_path.name}", leave=False):
            try:
                ...
                feat = extract_features(df, feature_names, kmer_set)
                X_list.append(feat)
...
# Training
# ============================================================================
def train_models(feature_names: List[str], kmer_set: set) -> Dict:
    ...
    for ds_id in range(1, 9):
        ...
        # Extract features
        X, ids, y = extract_dataset(train_path, feature_names, kmer_set, metadata, is_test=False)
        y = np.array(y)
...
# Prediction
# ============================================================================
def predict_test(models: Dict, feature_names: List[str], kmer_set: set) -> pd.DataFrame:
    ...
    for test_name, train_id in TEST_TO_TRAIN.items():
        ...
        # Extract features
        X_test, test_ids, _ = extract_dataset(test_path, feature_names, kmer_set, is_test=True)
...
def main():
    ...
    print(f"""
================================================================================
  AIRR-ML-25 Champion V4 - GPU-Optimized Feature Pipeline
  Started: {start.strftime('%Y-%m-%d %H:%M:%S')}

  FIX: Limiting k-mers to top {MAX_KMERS} (was ~150K causing GPU OOM)
  Features: K-mers (top {MAX_KMERS}) + V/J genes + Diversity metrics
================================================================================
""")
    ...
    models = train_models(feature_names, kmer_set)

    # Phase 3: Task A predictions
    task_a = predict_test(models, feature_names, kmer_set)
    ...
    path = SUBMISSION_DIR / f'champion_v4_{ts}.csv'
